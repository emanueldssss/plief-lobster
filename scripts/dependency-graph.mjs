...
import { insideRoot } from "./lib/contain.mjs";
...
const entry = resolve(project, entryArg);
if (!insideProject(entry)) { console.error(JSON.stringify({status: "ERROR", operation: "resolve-entry", path: entryArg, root: project, reason: "entry must stay inside the project root"})); process.exit(2); }
visit(entry);
